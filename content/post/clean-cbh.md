...
dbatools is different. Every function comes with extensive help including documentation of all parameters, examples for usage, and an explanation of what the function does, how and why. PowerShell's built-in [comment-based help system](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_comment_based_help) is tremendously helpful with developing this; write documentation into your function using the standard template, and it's instantly accessible with `Get-Help`.
...
1. Set up a GitHub account, fork the project, and clone it to your desktop. If you're new to the process, check out the [CONTRIBUTING.md guide](https://github.com/dataplat/dbatools/blob/development/CONTRIBUTING.md) in the dbatools repository to get started.
2. Make sure your editor's formatting is configured to be consistent with the rest of the project. If you use Visual Studio Code, check the `.vscode` directory in the dbatools repository for sample workspace settings files.
3. Take a look at the [dbatools-templates repository](https://github.com/dataplat/dbatools-templates) for samples of the CBH style that should be used (note: this repository is archived but available for reference). You can also look at functions in the main dbatools repository which have already been updated to the standard style.
