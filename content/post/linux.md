...
![TSQL2SDAY](/images/tsql2sday.png)
...
This month's T-SQL Tuesday is hosted by Tracy Boggiano ([blog](https://tracyboggiano.com/)), is all about Linux.

## dbatools and Linux

As a long-time Linux user and open-source advocate, I was beyond excited when [PowerShell](https://learn.microsoft.com/en-us/powershell/scripting/install/installing-powershell-core-on-linux) and [SQL Server](https://learn.microsoft.com/en-us/sql/linux/sql-server-linux-setup) came to Linux.
...
If you're curious, our [docs site](https://dbatools.io/commands) will tell you if a command is supported by Linux and Mac OS.
...
Interested in learning more? Check out the #1 rated SQL Server on Linux book, [Pro SQL Server on Linux](https://amzn.to/2YHPe4I) by Microsoft's Bob Ward. This book was actually edited by our friend [Anthony Nocentino](https://www.nocentino.com/blog/) and it's rumored that Anthony will contribute some sweet Linux-centric commands to dbatools when the Year of the Linux Desktop arrives, so we're looking forward to that 😊
...
Connecting to SQL Server (Windows or Linux) from Linux or Mac OS is generally done with using an alternative `-SqlCredential`. So let's say you follow Microsoft's guide to setting up [SQL on Linux](https://learn.microsoft.com/en-us/sql/linux/sql-server-linux-setup) or if you use dbatools' [Docker guide](https://dbatools.io/docker), you'll likely need to authenticate with the `sa` or `sqladmin` account. Execute the following command.
